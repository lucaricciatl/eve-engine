               ...
               int quality)
{
    ...
    // All COM objects must be released BEFORE CoUninitialize(), so we scope
    // them inside an inner block.  The lambda returns the success status.
    bool ok = [&]() -> bool {
        ...
        return true;
    }();  // ComPtr destructors run here, before CoUninitialize
