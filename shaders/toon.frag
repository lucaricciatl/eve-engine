...
void main() {
    ...
    for (int i = 0; i < lightCount; ++i) {
        ...
        float quantized = floor(diff * bands) / max(bands - 1.0, 1.0);
        vec3 contribution = baseColor * quantized * lightColor * lightIntensity * attenuation;

        if (i == 0) {
            float shadow = enableShadows ? computeShadow(N, lightDir) : 0.0;
            visibility = enableShadows ? (1.0 - shadow) : 1.0;
            toonLight += contribution * visibility;
        } else {
            toonLight += contribution;
        }
    ...
    vec3 color = ambient + toonLight + emissive;
