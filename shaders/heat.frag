...
void main() {
    ...
    float heightT = (fragWorldPos.y + 1.5) / 5.0;
    float distanceT = length(fragWorldPos.xz) / 10.0;
    float t = clamp(heightT + distanceT * 0.35, 0.0, 1.0);

    vec3 base = heatGradient(t) * (0.6 + 0.4 * sampledAlbedo.rgb);
